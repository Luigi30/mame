...
#define VERBOSE (LOG_FLOPPY|LOG_VMEBUS)
...
void vme_mvme320_device::device_start()
{
	...
	m_cl_timer = machine().scheduler().timer_alloc(timer_expired_delegate(FUNC(vme_mvme320_device::cl_timer_expired), this));
...
void vme_mvme320_device::mclk_w(uint8_t val)
{
	...
	else if(m_mclk_state == MCLK_ASSERTED && m_io_bank == IO_BANK_RB && m_io_phase == IO_PHASE_WC)
	{
		...
		switch(regDecoder.m_write_state)
		{
			case RegDecoder::WUASn: // IV bus is latched when /WUAS & /MCLK
				...
				m_vme_address = (m_vme_address & 0x0000FFFF) | (((uint32_t)inverted) << 16); // active low IV going into active high latch
				break;
			case RegDecoder::WUDSn: // IV bus is latched when /WUDS & /MCLK
				LOGDECODER("%04X: Writing WUDS: %02X\n", m_maincpu->pc(), m_iv_state);
				m_vme_data = (m_vme_data & 0x00FF) | (((uint32_t)m_iv_state) << 8);			// active low IV going into inverting latch
				break;
			case RegDecoder::WRDn:	// IV bus is latched when /WRD & /MCLK
				LOGDECODER("%04X: Writing Drive Status: %02X\n", m_maincpu->pc(), m_iv_state);
				m_drive_status = m_iv_state;												// active low floppy going into active low IV
				break;
			case RegDecoder::WLDSn: // IV bus is latched when /WLDS & /MCLK
				LOGDECODER("%04X: Writing WLDS: %02X\n", m_maincpu->pc(), m_iv_state);		
				m_vme_data = (m_vme_data & 0xFF00) | m_iv_state;							// active low IV going into inverting latch
				break;
			case RegDecoder::VCRn:	// IV bus is latched when /VCR & /MCLK
				// IV -> VME control reg
				update_vme_control_lines(inverted); 										// active low IV going into active high latch
				break;
			case RegDecoder::WMASn: // IV bus is latched when /WMAS &/ MCLK
				LOGDECODER("%04X: Writing WMAS: %02X\n", m_maincpu->pc(), m_iv_state);
				m_vme_address = (m_vme_address & 0x00FF00FF) | (((uint32_t)inverted) << 8); // active low IV going into active high latch
				break;
			case RegDecoder::WLASn: // IV bus is latched when /WLAS & /MCLK
				LOGDECODER("%04X: Writing WLAS: %02X\n", m_maincpu->pc(), m_iv_state);
				m_vme_address = (m_vme_address & 0x00FFFF00) | inverted; 					// active low IV going into active high latch
				break;
			case RegDecoder::WDC1n:	// IV bus is latched when /WDC1 & /MCLK
				...
				m_disk_control_1 = (m_iv_state & 0xFB) | (m_drive_status & 0x04); // IV5 is not connected, it comes from Disk Status Reg
				update_disk_control_1(m_iv_state); 											// active low IV going into active low floppy
				break;
			case RegDecoder::WDC2n: // IV bus is latched when /WDC2 & /MCLK
				...
				update_disk_control_2(m_iv_state);											// active low IV going into active low floppy
				break;
			case RegDecoder::WDC3n: // IV bus is latched when /WDC3 & /MCLK
				// Write Disk Control 3 (U58)
				vme_dbc3_line_log(tmp, inverted);
				LOGDECODER("%04X: writing Disk Control 3: %02X (%s)\n", m_maincpu->pc(), inverted, tmp);
				m_disk_control_3 = inverted;
				update_disk_control_3(inverted);											// active low IV going into active high latch
				break;
			case RegDecoder::WDBCn:
				if(m_iv_state != 0) LOGFLOPPY("%04X MVME WDBC: %02X\n", m_maincpu->pc(), m_iv_state);
				m_dbcr->write_iv(~m_iv_state); // this gets inverted
				m_dbcr->wc_w(1);
				m_dbcr->me_w(1);
				m_dbcr->mclk_w(1);
				m_dbcr->mclk_w(0);
			...
			case RegDecoder::WBUn:
				...
				ram_bank_w(m_disk_buffer_address_latch, m_iv_state);							// active low IV going into active high SRAM
				break;
			default:
				break;
		}
	...
	if(m_mclk_state == MCLK_ASSERTED && m_io_bank != IO_BANK_RB && m_io_phase == IO_PHASE_SC)
	{
		// LB Address phase
...
uint8_t vme_mvme320_device::vme_read_lobyte()
{
	...
	LOGVMEBUS("%04X: MVME320 reading VMEbus [D07:D00], %08X %02X\n", m_maincpu->pc(), m_vme_address << 1, m_vme_data & 0xFF);
	return m_vme_data & 0xFF;
}
...
uint8_t vme_mvme320_device::vme_read_hibyte()
{
	...
	LOGVMEBUS("%04X: MVME320 reading VMEbus [D15:D08], %08X %02X\n", m_maincpu->pc(), m_vme_address << 1, m_vme_data >> 8);
...
{
	LOGVMEBUS("%04X: MVME320 writing VMEbus [D07:D00], %08X %02X\n", m_maincpu->pc(), m_vme_address << 1, m_vme_data & 0xFF);
...
{
	LOGVMEBUS("%04X: MVME320 writing VMEbus [D15:D07], %08X %02X\n", m_maincpu->pc(), m_vme_address << 1, m_vme_data >> 8);
	m_vme_status_2 |= VSR2_CYACTIV;
	...
	if(m_io_phase == IO_PHASE_NONE)
	{
		...
		switch(regDecoder.m_read_state)
		{
			case RegDecoder::VSR1n:
				m_iv_state = bitswap<8>(m_vme_status_1, 0, 1, 2, 3, 4, 5, 6, 7); // swapped data lines
				LOGDECODER("%04X: MCU reading VSR1, value %02X\n", m_maincpu->pc(), m_iv_state); // inverting latch -> active-low IV bus
				break;
			case RegDecoder::RDBCn:
				m_dbcr->me_w(1);
				m_dbcr->rc_w(1);
				m_iv_state = m_dbcr->read_iv();									//  IV -> UD is inverted, IV -> IV is not
				m_dbcr->rc_w(0);
				m_dbcr->me_w(0);
				LOGFLOPPY("%04X: MCU reading DBC, value %02X\n", m_maincpu->pc(), m_iv_state);
				break;
			case RegDecoder::VRDLn: // Reads the register, NOT the VMEbus. 
				m_iv_state = m_vme_data & 0xFF; 								// active-high VME -> inverting latch -> active-low IV bus
				LOGDECODER("%04X: MCU reading VRDL, value %02X\n", m_maincpu->pc(), m_iv_state);
				break;
			case RegDecoder::RBUn:
				m_iv_state = ram_bank_r(m_disk_buffer_address_latch);			// active-high SRAM -> active-low IV bus
			...
			case RegDecoder::VSR2n:
				...
				// /LBERR 	- b2 - IV1 (IV6)
				// CYACTIV 	- b3 - IV0 (IV7)
				m_iv_state = bitswap<4>(~m_vme_status_2, 3, 2, 1, 0) << 4; 		// active-high latch -> active-low IV bus
			...
			case RegDecoder::RDSn:	// Output enabled when /RB & /RDS.
				...
				m_iv_state = get_floppy_state(0);								// active-low floppy -> active-low IV bus
				if(ds != m_iv_state) { ds = m_iv_state; LOGFLOPPY("%04X: MCU reading DS, value %02X\n", m_maincpu->pc(), m_iv_state); }
				break;
			case RegDecoder::VRDUn: // Reads the register, NOT the VMEbus.
				m_iv_state = m_vme_data >> 8; 									// active-high VME -> inverting latch -> active-low IV bus
...
{
	static uint8_t old;
	// active-low IV bus -> active-low floppy connector
	char signals[256] = "";
	if(BIT(value, 0)) strcat(signals, "WRGATE ");
	if(BIT(value, 1)) strcat(signals, " WGTE ");
	if(BIT(value, 2)) strcat(signals, " FMODE ");
	if(BIT(value, 3)) strcat(signals, " UNITSEL0 ");
	if(BIT(value, 4)) strcat(signals, " SEL1 ");
	if(BIT(value, 5)) strcat(signals, " SEL2 ");
	if(BIT(value, 6)) strcat(signals, " SEL3 ");
	if(BIT(value, 7)) strcat(signals, " SEL4");

	if(old != value) { old = value; LOGFLOPPY("%04X: clocking disk control 1: %02X (%s)\n", m_maincpu->pc(), value, signals); }
	// should FMODE be high or low?

	floppy_image_device *floppy;
		 if(BIT(m_disk_control_1, 4)) { floppy = m_fd0->get_device(); }
	else if(BIT(m_disk_control_1, 5)) { floppy = m_fd1->get_device(); }
	else if(BIT(m_disk_control_1, 6)) { floppy = m_fd2->get_device(); }
	else if(BIT(m_disk_control_1, 7)) { floppy = m_fd3->get_device(); }
	else return;
	...
	floppy->mon_w(!BIT(value, 3)); // active-low
	m_cl_timer->adjust(attotime::from_hz(250000));
	m_cl_timer->enable();
}
...
void vme_mvme320_device::update_disk_control_2(uint8_t value)
{
	...
	// active-low IV bus -> active-low floppy connector
	char signals[256] = "";
	if(BIT(value, 0)) strcat(signals, "HEAD20 ");
	if(BIT(value, 1)) strcat(signals, " HD20/SIDE ");
	if(BIT(value, 2)) strcat(signals, " HEAD21 ");
	if(BIT(value, 3)) strcat(signals, " HEAD22 ");
	if(BIT(value, 4)) strcat(signals, " STEP ");
	if(BIT(value, 5)) strcat(signals, " STEP1SECT ");
	if(BIT(value, 6)) strcat(signals, " DIR ");
	if(BIT(value, 7)) strcat(signals, " LOWCURRENT");

	if(old != value) { old = value; LOGFLOPPY("%04X: clocking disk control 2: %02X (%s)\n", m_maincpu->pc(), value, signals); }
	...
	// 0x08		HEAD22		X		4
	// 0x04		HEAD21		X		18
	// 0x02		HD20/SIDE	32		48
	// 0x01		HEAD20		X		14

	floppy->dir_w(BIT(value, 6));
	floppy->stp_w(BIT(value, 4));
	floppy->ss_w(BIT(value, 1));
}
...
void vme_mvme320_device::update_disk_control_3(uint8_t value)
{
	static uint8_t old;
	char signals[256] = "";
	if(BIT(value, 0)) strcat(signals, "/SDR ");
	if(!BIT(value, 1)) strcat(signals, " U64.A7 ");
	if(!BIT(value, 2)) strcat(signals, " CIRQ ");
	if(!BIT(value, 3)) strcat(signals, " PRECOMP ");
	if(!BIT(value, 4)) strcat(signals, " ECC ");
	if(!BIT(value, 5)) strcat(signals, " MS2 ");
	if(!BIT(value, 6)) strcat(signals, " MS1 ");
	if(!BIT(value, 7)) strcat(signals, " FDHD");

	if(old != value) { old = value; LOGFLOPPY("%04X: clocking disk control 3: %02X (%s)\n", m_maincpu->pc(), ~value, signals); }
	update_u81_speeds();
...
void vme_mvme320_device::update_u81_speeds()
{
	uint8_t MS = m_disk_control_3 & 0x60;

	switch(MS)
	{
		case 0x60: 	// neither
			LOGFLOPPY("CR = 10MHz, CL = VCO\n");
			break;
		case 0x40:	// MS2
			LOGFLOPPY("CR = 1MHz, CL = VCO/8\n");
			break;
		case 0x20:	// MS1
			LOGFLOPPY("CR = 500KHz, CL = VCO/16\n");
			break;
		case 0x00:	// both
			LOGFLOPPY("CR = 250KHz, CL = VCO/32\n");
			break;
	}
}
...
uint8_t vme_mvme320_device::get_floppy_state(int floppy_num)
{
	...
	char signals[256] = "";
	if(seekcompl) strcat(signals, "SEEKCOMPL ");
	if(ix) strcat(signals, "IX ");
	if(ready) strcat(signals, "READY ");
	if(twosided) strcat(signals, "TWOSIDED ");
	if(tr000) strcat(signals, "TR000 ");
	if(notbusy) strcat(signals, "NOTBUSY ");
	if(diskchange) strcat(signals, "DISKCHANGE ");
	if(wrfault) strcat(signals, "WRFAULT ");

	retval = ~((seekcompl << 7) | (ix << 6) | (ready << 5) | (twosided << 4) | (tr000 << 3) | (notbusy << 2) | (diskchange << 1) | wrfault);

	if(old != retval) { old = retval; LOGFLOPPY("reading DS, state changed: %02X (%s)\n", retval, signals); }
	return retval;
}
...
uint8_t vme_mvme320_device::ram_bank_r(offs_t offset)
{
	...
	offset += (m_dbcr->read_ud() & 0x03) * 256;
 ...
 * Drive Sequencer
 ************************/
TIMER_CALLBACK_MEMBER(vme_mvme320_device::cl_timer_expired)
{
	drive_sequencer_clock();
}
...
#define DISABLE_ALL_READS { VSR1n_w(0); RDBCn_w(0); VRDLn_w(0); RBUn_w(0); VSR2n_w(0); RDSn_w(0); VRDUn_w(0); }
/************************
 * Decoder Read Lines
 ************************/
 ...
 * Decoder Write Lines
 ************************/
#define DISABLE_ALL_WRITES { WUASn_w(0); WUDSn_w(0); WRDn_w(0); WLDSn_w(0); VCRn_w(0); WMASn_w(0); WLASn_w(0); WDC1n_w(0); WDBCn_w(0); WDC3n_w(0);WDC2n_w(0); WBUn_w(0); }
